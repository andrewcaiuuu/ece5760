...
int main(void)
{
	...
	// int rho = (int) *(fpga_rho_ptr) / DIVISION_CONST;
	// int beta = (int) * (fpga_beta_ptr) / DIVISION_CONST;
	// int sigma = (int) * (fpga_sigma_ptr) / DIVISION_CONST;
	// char rho_str[20];
	// char beta_str[20];
	// char sigma_str[20];
	// sprintf(rho_str, "rho = %d\0", rho);
	// sprintf(beta_str, "beta = %d\0", beta);
	// sprintf(sigma_str, "sigma = %d\0", sigma);

	// char text_top_row[40] = rho_str;
	// char text_bottom_row[40] = beta_str;
	// char text_next[40] = sigma_str;
	...
	while(1){
		...
		if (input == 'r'){
			...
			*(fpga_sigma_ptr) = fixed_sigma;
			...
			int new_rho = (int) *(fpga_rho_ptr) / DIVISION_CONST;
			int new_beta = (int) * (fpga_beta_ptr) / DIVISION_CONST;
			int new_sigma = (int) * (fpga_sigma_ptr) / DIVISION_CONST;
			char rho_str[20];
			char beta_str[20];
			char sigma_str[20];
