...
void optimise_fitness(int *image, int *weight, int *previous_edge, struct Tuple *xy){
    ...
    for (int i = 0; i < TIME_SAVER; i++){
        ...
        while (current != NULL){
            ...
            int weight_data = weight[ temp->data.x + (temp->data.y) * COLS ];
...
int main(){
    ...
    // // DEBUG______________________________
    // struct LinkedList* test = initLinkedList();
    // struct Tuple start;
    // struct Tuple end;
    // start.x = 11;
    // start.y = 167;
    ...
    // // DEBUG______________________________
    // int *ah_weights = (int *)calloc(ROWS * COLS, sizeof(int));
    // // DEBUG______________________________
